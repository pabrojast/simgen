...
setwd(pathpreproc)
tmin <- read.csv(file=paste0(pathpreproc,"_tmin.csv"))
tmax <- read.csv(file=paste0(pathpreproc,"_tmax.csv"))
# Leer el archivo CSV
datos <- tmin[1]
datos2 <- tmax[1]
...
convertir_fecha <- function(fecha) {
  ...
  year <- substr(fecha, 2, 5)
  month <- substr(fecha, 7, 8)
  day <- substr(fecha, 10, 11)
  
  # Creamos la fecha en formato yyyy-mm-dd
  fecha_formateada <- paste(year, month, day, sep = "-")
  
  return(fecha_formateada)
}

# Aplicamos la función a la columna de fechas y guardamos los resultados en una nueva columna
datos$fecha_formateada <- sapply(datos, convertir_fecha)
datos2$fecha_formateada <- sapply(datos2, convertir_fecha)

# Eliminamos la columna original si ya no es necesaria
tmin[1] <- datos[2]
tmax[1] <- datos2[2]
#formateo
tmin[1] <- as.Date( tmin$X, "%Y-%m-%d")
tmax[1] <- as.Date( tmax$X, "%Y-%m-%d")


formateo <- function(id){
  print(paste("Writting file: ",id))
  #leemos precipitacion
  pp <- read.csv(paste("./BWA/est_procesadas/",id,"_latam.txt",sep=""),sep=";")
  pp$date <-as.Date( pp$date, "%Y-%m-%d")
  
  #same lenght
  tminss <- subset(tmin, X >= pp$date[1] )
  tminss <- subset(tminss, X <= tail(pp$date,n=1))
  tmaxss <- subset(tmax, X >= pp$date[1] )
  tmaxss <- subset(tmaxss, X <= tail(pp$date,n=1))
  
  
  year <- substr(tminss$X, 1, 4)
  month <- substr(tminss$X, 6, 7)
  day <- substr(tminss$X, 9, 10)
  ...
  df <- data.frame(
    ...
    month = month,
    day = day,
    "precip" =as.numeric(pp["prcp_chirps"][[1]]),
    "tmax" = as.numeric(tmaxss[id][[1]]),
    "tmin" = as.numeric(tminss[id][[1]])
  )
  #definimos nan
  df[is.na(df)] <- -999
  #escribimos
  write.table(df,paste("./output/",id,".csv",sep=""),row.names = FALSE, dec=".", sep=";")
}

base::lapply(stations_metadata$id_station, formateo )
