...
def gen(obsix=[2000],simix=2955,trendq=0.5,write=1,fname='sim_100kyr.dat',\
        simlen=62, locate=2041, xval=0, M=1, trendmean=-0.125968, trendsd=1.76931, temptrendmean=3.97, temptrendsd=1.26):
    """ Main routine """
...
# First chore: Assemble the necessary files...
    ...
    tmmm = np.loadtxt('pickled/tasa_cmip_1901-2095.txt')
    #############################!fin de modificacion: 17-02-2017
    tmmm = pickle.load(open('pickled/tasav_cmip5_comb_sm_1901-2095.p'))
    print 'File is read!'
    sys.stdout.flush()
...
#    trendsd   =  1.76931
    ...
    fm21tmax = norm.ppf(1-trendq,temptrendmean,temptrendsd)
    fm21tmin = norm.ppf(1-trendq,temptrendmean,temptrendsd)
...
# First, fetch the (non-detrended) pr...
    ...
    fm20tmax = lm(x20c,obsmi_raw) 
    fm20tmin = lm(x20c,obsma_raw) 
    #modificado 21-09-2017 fm20[0]*0 para evitar problemas en estaciones con tendencias muy marcadas.
    fm2000 = 0.5 * (fm20[0]*0 + fm21)          # For the 2000-2005 "transition"
    fm2000tmax = 0.5 * (fm20tmax[0]*0 + fm21tmax) 
    fm2000tmin = 0.5 * (fm20tmin[0]*0 + fm21tmin) 
...
# feature vectors.
    ...
    ##################modificado 16-02-2017################################
    #tmax
    M0 = obsmi_raw[-10:].mean()              # Mean of final 10 yr of obs
    x0 = np.log(M0) - fm2000tmax*x21c[0]         # Match level M0
    mi21forced[:6] = np.exp(fm2000tmax*x21c[:6] + x0) # 2000-2005, inclusive
    M1 = 0.1*(obsmi_raw[-4:].sum()+mi21forced[:6].sum()) # Mean, 1996-2005
    x1 = np.log(M1) - fm21tmax*x21c[6]           # Shift, match again...
    mi21forced[6:] = np.exp(fm21tmax*x21c[6:] + x1)   # Based on trendq...
    print "mi21forced"+str(mi21forced)    
    #tmin
    M0 = obsma_raw[-10:].mean()              # Mean of final 10 yr of obs
    x0 = np.log(M0) - fm2000tmin*x21c[0]         # Match level M0
    ma21forced[:6] = np.exp(fm2000tmin*x21c[:6] + x0) # 2000-2005, inclusive
    M1 = 0.1*(obsma_raw[-4:].sum()+ma21forced[:6].sum()) # Mean, 1996-2005
    x1 = np.log(M1) - fm21tmin*x21c[6]           # Shift, match again...
    ma21forced[6:] = np.exp(fm21tmin*x21c[6:] + x1)   # Based on trendq...
    print "mi21forced"+str(ma21forced)    
    #forma antigua de generar proyeccion
    #fmma = lm(x20c,obsma_raw)                # Fit to 20C
    #fmmi = lm(x20c,obsmi_raw)
    #print "fmma: "+str(fmma)+" fmmi"+str(fmmi)
    ...
    if xval == 0:
        ...
        ############modificado 16-02-2017##########################################
        #tmin
        M0 = obsma_raw[:10].mean()           # First 10 values...
        x0 = np.log(M0) - fm20tmin[0]*x20c[0]

        ma20forced = np.exp(fm20tmin[0]*x20c + fm20tmin[1])    # Simpler = better!
        ma20total  = ma20total + simma[:infoSerie.lenSerie] # Same as done for catchments

        #tmax
        
        M0 = obsmi_raw[:10].mean()           # First 10 values...
        x0 = np.log(M0) - fm20tmax[0]*x20c[0]

        mi20forced = np.exp(fm20tmax[0]*x20c + fm20tmax[1])    # Simpler = better!
        mi20total  = mi20total + simmi[:infoSerie.lenSerie] # Same as done for catchments
...
# not altered.
        yrpr,breaks0 = yrgen(pr, dummy)             # With trend and mean; not log
        yrma,breaks1 = yrgen(tmax, dummy)           # All breaks should be the same
        yrmi,breaks2 = yrgen(tmin, dummy)
        obslen = len(yrpr)
        #fix, because if yrpr = 0, log(yrpr) -> inf. //21-09-2017
        yrpr[yrpr == 0] = 0.0027397260273973
        fmpr = lm(x20c,np.log(yrpr))         # Regression of (log) station pr,
        #fmpr = lm(x20c,res)         # Regression of (log) station pr,
        prforced = np.exp(fmpr[0]*xmm + fmpr[1]) # 100 (116) yr of CC trend.
        fmpr2 = lm(x20c,yrma)
        tmaxforced = fmpr2[0]*xmm + fmpr2[1]

        fmpr3 = lm(x20c,yrmi)
        tminforced = fmpr3[0]*xmm + fmpr3[1]
...
#
        alpha = 0                         # Fraction of local response
        fm21local = fmpr[0]                  # 21C imposed
        fm21x = fm21 + alpha*fm21local      # Weighted mixture fm21x = fm21 + alpha*fm21local  
        fm2000x = 0.5*(fm21local + fm21x)    # 2000-2005 transition
        if(fm2000x>0):#modificado en 21/09/2017 por pablo
            fm2000x =  fm21x*0.5
        #tmax
        fm21local2 = fmpr2[0]
        fm21x2 = fm21tmax + alpha*fm21local2 #fm21tmax + alpha*fm21local2
        fm2000x2 = 0.5*(fm21local2*alpha + fm21x2) #fm21local2*alpha modificado para que sea mas uniforme
        #tmin
        fm21local3 = fmpr3[0]
        fm21x3 = fm21tmin + alpha*fm21local3 #fm21tmin + alpha*fm21local3
        fm2000x3 = 0.5*(fm21local3*alpha + fm21x3) #fm21local3*alpha modificado para que sea mas uniforme.
        if(fm2000x3<0):#modificado por pablo 21/09/2017
            fm2000x3 =  fm21x3*0.5
        print 'Imposed pr trend:', fm21
        print 'Imposed tmax trend: ', fm21tmax
        print 'Imposed tmin trend: ', fm21tmin
        print 'Local tmax trend: ', fm21local2
        print 'Local tmin trend: ', fm21local3
        print 'Local pr trend:', fm21local
        print 'Weighted local trend departure:', alpha*fm21local
        print 'trend pr for 2000-2005 (transitional):', '%6.4f' % (fm2000x)
        print 'trend tmin 2000-2005:', '%6.4f' % (fm2000x3)
        print 'trend tmax 2000-2005:', '%6.4f' % (fm2000x2)
        print 'trend pr for 2005-2065:', '%6.4f' % (fm21x)
        print 'trend tmin 2005-2065:','%6.4f' % (fm21x3)
        print 'trend tmax 2005-2065:','%6.4f' % (fm21x2)
        print 'obsno: ', obsno
        print 'simix: ', simix
...
#
        ...
        #rectificar
        M0 = prforced[(obslen-10):obslen].mean()                  # For smooth join with 20c forced
        ...
        ma21forced = 1.0E+20 * np.ones(simlen)# Create array...
        M0 = tmaxforced[(obslen-10):obslen].mean()   # For smooth join with 20c forced
        x0 = np.log(M0) - fm2000x2*x20c[-1]    # Last value of x20c...
        ma21forced[:7] = np.exp(fm2000x2*x21c[:7] + x0) # 2000-2006, inclusive
        
        M1 = ma21forced[6]                    # Overwrite [6], continue...
        x1 = np.log(M1) - fm21x2*x21c[6]       # New join at 2006
        ma21forced[6:] = np.exp(fm21x2*x21c[6:] + x1)   # Based on trendq...
        
        tmaxforced[infoSerie.lenSerie:] = ma21forced            # Fill in 21c part of forced
        maforced = tmaxforced
        ##########################################->
        mi21forced = 1.0E+20 * np.ones(simlen)# Create array...
        M0 = tminforced[(obslen-10):obslen].mean()                     # For smooth join with 20c forced
        x0 = np.log(M0) - fm2000x3*x20c[-1]    # Last value of x20c...
        mi21forced[:7] = np.exp(fm2000x3*x21c[:7] + x0) # 2000-2006, inclusive
        M1 = mi21forced[6]                    # Overwrite [6], continue...
        x1 = np.log(M1) - fm21x3*x21c[6]       # New join at 2006
        
        mi21forced[6:] = np.exp(fm21x3*x21c[6:] + x1)   # Based on trendq...
        tminforced[infoSerie.lenSerie:] = mi21forced            # Fill in 21c part of forced
        miforced = tminforced 
        ##################modificado 16-02-2017################################            
        ##################modificado 16-02-2017################################
...
# Assemble trivariate arrays, for writing out and for returning.
        ...
        if write:
            ...
            #agregado 14-02-2017
            mensual_csv(obsix,simix,trendq)
            anual_csv(obsix,simix,trendq)
        else:
...
def getmoda(leap):
    ...
    for i in range(len(days)):
        for j in range(1,days[i]+1):
            datelist.append(mos[i]+str(',')+str(j).rjust(2))
...
def mensual_csv(obsix,simix,trendq):
    import glob
    import pandas as pd
    import datetime
    strtrendq = str(trendq)
    # config
    path = "output_sim/"
    dir_salida = "mensual/"
    ####
    ...
    def remove_duplicates(l):
        return list(set(l))
    os.chdir(path)
    existedir(dir_salida+"/"+str(int(trendq*100)))
    existedir(dir_salida)
    #generamos una nueva lista
    simt=[]
    estat=[]
    temp=[]
    fix=1 #para generar el index con el primer archivo
    for file in glob.glob("sim_100k_obshis_*.txt"):
        simt.append(file[-10:-4])
        estat.append(file[16:20])
    sim = remove_duplicates(simt)
    esta = remove_duplicates(estat)
    for i in range(0,len(obsix)):
        for file in glob.glob("sim_100k_obshis_"+str(obsix[i])+"_"+"*"+str(simix)+".txt"):
            var = pd.read_csv(file, sep=',', header=None)
            #var.columns = ["year", "month", "day", "pp"+str(obsix[i]), "tmax"+str(obsix[i]), "tmin"+str(obsix[i])]
            inicio = str(var[0][1])+str("-0")+str(var[1][0])+str("-0")+str(var[2][0])
            print 'Working file '+str(file)
            #note var[0] = yeat
            #var[1] = month
            #var[2] = day
            #var[3] = pp
            #var[4] = tmax
            #var[5] = tmin
            index = pd.date_range(inicio, periods=len(var), freq='D')
            var.index = index
            esta_current = file[16:20]
            if fix == 1:
                #generamos los dataframe
                print "Working DataFrame..."
                pp = pd.DataFrame(index=index, columns=esta)
                tmin = pd.DataFrame(index=index, columns=esta)
                tmax = pd.DataFrame(index=index, columns=esta)
                tprom = pd.DataFrame(index=index, columns=esta)
                fix = 2
            pp[str(esta_current)] = var[3]
            tmin[str(esta_current)] = var[5]
            tmax[str(esta_current)] = var[4]
            print trendq
            print trendq
            print trendq
            pp.resample('M').sum().to_csv(str(dir_salida)+str(int(trendq*100))+"/pp_"+str(simix)+".csv")
            tmin.resample('M').mean().to_csv(str(dir_salida)+str(int(trendq*100))+"/tmin_"+str(simix)+".csv")
            tmax.resample('M').mean().to_csv(str(dir_salida)+str(int(trendq*100))+"/tmax_"+str(simix)+".csv")
            tprom[str(esta_current)] = (tmin[str(esta_current)]+tmax[str(esta_current)])/2
            tprom.resample('M').mean().to_csv(str(dir_salida)+str(int(trendq*100))+"/tprom_"+str(simix)+".csv")
    os.chdir(current_dir)
def anual_csv(obsix,simix,trendq):
    import glob
    import pandas as pd
    import datetime
    strtrendq = str(trendq)
    ...
    for i in range(0,len(obsix)):
        for file in glob.glob("sim_100k_obshis_"+str(obsix[i])+"_"+"*"+str(simix)+".txt"):
            ...
            if fix == 1:
                #generamos los dataframe
                print "Working yearly DataFrame..."
                pp = pd.DataFrame(index=index, columns=esta)
                tmin = pd.DataFrame(index=index, columns=esta)
                tmax = pd.DataFrame(index=index, columns=esta)
                tprom = pd.DataFrame(index=index, columns=esta)
            ...
            tmin[str(esta_current)] = var[5]
            tmax[str(esta_current)] = var[4]
            pp.resample('A').sum().to_csv(str(dir_salida)+str(int(trendq*100))+"/pp_"+str(simix)+".csv")
            tmin.resample('A').mean().to_csv(str(dir_salida)+str(int(trendq*100))+"/tmin_"+str(simix)+".csv")
            tmax.resample('A').mean().to_csv(str(dir_salida)+str(int(trendq*100))+"/tmax_"+str(simix)+".csv")
            tprom[str(esta_current)] = (tmin[str(esta_current)]+tmax[str(esta_current)])/2
            tprom.resample('A').mean().to_csv(str(dir_salida)+str(int(trendq*100))+"/tprom_"+str(simix)+".csv")
    os.chdir(current_dir)
########################### T H E ### E N D #############
########################################################
###################SIMGEN WINDOWS########################
...
arg8 = int(sys.argv[8])
arg9 = int(sys.argv[9])
arg10 = float(sys.argv[10])
arg11 = float(sys.argv[11])
arg12 = float(sys.argv[12])
arg13 = float(sys.argv[13])
#####################################################

gen(arg1, arg2, arg3, write=arg4, fname=arg5, simlen=arg6, locate=arg7, xval=arg8, M=arg9, trendmean=arg10, trendsd=arg11, temptrendmean= arg12, temptrendsd=arg13)
